...
function sendMoney(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = mcdcJS.transaction.createTransaction(
    options.to,
...
function registerDelegate(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = mcdcJS.delegate.createDelegate(
    options.username,
    options.secret,
    options.secondSecret
  );
...
function vote(secret, publicKeys, op, secondSecret) {
  ...
  var trs = mcdcJS.vote.createVote(
    votes,
    secret,
    secondSecret
  );
...
function setSecondSecret(options) {
  var trs = mcdcJS.signature.createSignature(options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.success);
  });
}
...
function registerDapp(options) {
  ...
  var trs = mcdcJS.dapp.createDapp(dapp, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
...
function registerAsset(options) {
  ...
  var trs = mcdcJS.uia.createAsset(payload, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
function registerIssuer(options) {
  var trs = mcdcJS.uia.createIssuer(options.name, options.desc, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
function submitApproval(options) {
  var trs = mcdcJS.uia.createApproval(Number(options.topic), options.value, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
function issueAsset(options) {
  var trs = mcdcJS.uia.createIssue(options.currency, options.amount, options.rate, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
...
function exercise(options) {
  var trs = mcdcJS.uia.createExercise(options.currency, options.amount, options.message, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId);
  });
}
